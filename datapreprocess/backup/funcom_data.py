...
"""
This is file to build our dataset.pkl from leclair raw data 
"""
...
def read_train_data(file_path):
    ...
    for k, v in sbt.items():
        tok_word_count = count_word(tok_word_count,
                                    split_identifier(toks[k])[:config.code_len])  # query
        sum_word_count = count_word(sum_word_count,
                                    split_identifier(sums[k])[:config.summary_len])  # answer
        sbt_word_count = count_word(sbt_word_count, v[:config.sbt_len])  # answer
        lines.append({"code_tokens":toks[k], "docstring_tokens":sums[k], "SBT":v})


    print('finished process lines')
    # build vocabulary
...
if __name__ == "__main__":
    ...
    tok_path = '/datadrive/yuxuan_data/funcom/funcom_tokenized/functions'
    sum_path = '/datadrive/yuxuan_data/funcom/funcom_tokenized/comments'
    sbt_path = '../sbt_data.json'
    # main
