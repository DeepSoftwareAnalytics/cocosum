...
def get_opt():
    ...
    parser.add_argument('-rnn_size', type=int, default=256, help='Size of LSTM hidden states')
    parser.add_argument('-word_vec_size', type=int, default=256, help='Word embedding sizes')
    ...
    parser.add_argument('-batch_size', type=int, default=256, help='Maximum batch size')
...
def load_data(opt):
    ...
    dataset["train_pg"]['trees'] = dataset["train_xe"]['trees']
    ...
    dataset["train_pg"]['leafs'] = dataset["train_xe"]['leafs']
...
def create_critic(checkpoint, dicts, opt):
    if opt.load_from is not None and "critic" in checkpoint:
        ...
        state = checkpoint["critic_optim"].optimizer.state
        for p in state:
            state[p]['exp_avg_sq'] = state[p]['exp_avg_sq'].cuda(opt.gpus[0])
            state[p]['exp_avg'] = state[p]['exp_avg'].cuda(opt.gpus[0])
        critic_optim.optimizer.state = state
...
def main():
    ...
    if opt.load_from is None:
        ...
        optim = checkpoint["optim"]
        state = checkpoint["optim"].optimizer.state
        for p in state:
            state[p]['exp_avg_sq'] = state[p]['exp_avg_sq'].cuda(opt.gpus[0])
            state[p]['exp_avg'] = state[p]['exp_avg'].cuda(opt.gpus[0])
        optim.optimizer.state = state
    ...
    # writer = SummaryWriter(log_dir='./log', comment='hybrid')
    # with writer:
    #     writer.add_graph(model, (supervised_data,))
