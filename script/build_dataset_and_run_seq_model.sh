#!/usr/bin/env bash
cd ../
mkdir -p "./output_diff_clen_dvoc10000_cvoc10000_svoc10000_refilter_summary"
path="output_diff_clen_dvoc10000_cvoc10000_svoc10000_refilter_summary"
cd $path
...
dataset_path="/mnt/Data/csn/dataset/diff_clen_refilter_summary"
...
#                                                   log_path = "$log_path"
                                                   ...
                                                   python ../runSeqModel.py  -dlen "$dlen"  -clen  "$clen" -slen "$slen" \
                                                        -dvoc "$dvoc" -cvoc "$cvoc"  -svoc "$svoc" \
                                                        -djl "$djl"  -dfp "$dfp" -dsi "$dsi"  -dlc "$dlc" -dr "$dr" \
                                                        ...
                                                        -rnn_hidden_size "$rhs" -lr "$lr" -epoch "$epoch" | tee "$log_path"
