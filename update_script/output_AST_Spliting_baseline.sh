#!/usr/bin/env bash
cd ../
mkdir -p "./output_AST_Spliting_baseline"
path="output_AST_Spliting_baseline"
cd $path
...
dataset_path="/mnt/enshi/CodeSum/data/csn/dataset/"
docstring_tokens_split_path="../data/csn/summary/cfp1_csi1_cfd0_clc1.pkl"
batch_size="256"

modeltype='ast-att-gru att-gru codenn h-deepcom'   # 'ast-att-gru', 'att-gru', 'codenn', 'uml', 'h-deepcom'
...
#                                                   log_path = "$log_path"
                                                   ...
                                                   python ../runGruUml.py  -dlen "$dlen"  -clen  "$clen" -slen "$slen" \
                                                        -dvoc "$dvoc" -cvoc "$cvoc"  -svoc "$svoc" \
                                                        -djl "$djl"  -dfp "$dfp" -dsi "$dsi"  -dlc "$dlc" -dr "$dr" \
                                                        -cfp "$cfp" -csi "$csi"  -cfd "$cfd"  -dataset_path "$dataset_path"  -docstring_tokens_split_path "$docstring_tokens_split_path"  \
                                                        -gpu_id "$gpu_id"   -modeltype "$mt" \
                                                        -batch_size "$bs" -code_dim "$code_tok_ed" -summary_dim "$sum_tok_ed" -sbt_dim "$sbt_tok_ed" \
                                                        -rnn_hidden_size "$rhs" -lr "$lr"  \
