...
class ASTAttnGRUDecoder(nn.Module):
    def __init__(self, vocab_size, emd_dim, hidden_size):
        ...
        self.query_trans_name = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Dropout(p=0.6),
                                     nn.ReLU(inplace=True))
        self.query_trans_uml =nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Dropout(p=0.6),
                                     nn.ReLU(inplace=True))
        self.value_trans_name = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Dropout(p=0.6),
                                     nn.ReLU(inplace=True))
        self.value_trans_uml = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Dropout(p=0.6),
                                     nn.ReLU(inplace=True))
