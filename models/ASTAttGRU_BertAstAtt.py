...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        context = torch.cat((ast_context, class_feature), dim=1)
        #pdb.set_trace()
        ...
        context_attn = F.softmax(context_attn, dim=2)  # [batch_size, 1, 2]
        # [batch_size, 1, 2] * [batch size, 2, hidden_size] = [batch_size, 1, hidden_size]
        context = torch.bmm(context_attn, context)
        
        
        final_context = torch.cat((context, txt_context, expand_hidden_states), dim=2)
