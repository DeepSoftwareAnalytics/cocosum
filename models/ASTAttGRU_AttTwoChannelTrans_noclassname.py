...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        #                         dim=1)
        class_query = self.query_trans_uml(class_encoder_output)
        class_value = self.value_trans_uml(class_encoder_output)

        # pdb.set_trace()
        # [batch_size, 1, hidden_size] * [batch size, hidden_size, 2] = [batch_size, 1, 2]
        # [batch_size, 1, hidden_size] * [batch size, hidden_size, 1] = [batch_size, 1, 1]
        class_context_attn = torch.bmm(expand_hidden_states, class_query.permute(0, 2, 1))
        class_context_attn = F.softmax(class_context_attn, dim=2)  # [batch_size, 1, 2] [batch_size, 1, 1]
        # [batch_size, 1, 2] * [batch size, 2, hidden_size] = [batch_size, 1, hidden_size]
        # [batch_size, 1, 1] * [batch size, 1, hidden_size] = [batch_size, 1, hidden_size]
