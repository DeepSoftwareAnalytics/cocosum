...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
                code_encoder_output, class_feature, prev_hidden_states):
        ...
        context = torch.cat((txt_context, ast_context, class_feature), dim=1)
        #pdb.set_trace()
        # [batch_size, 1, hidden_size] * [batch size, hidden_size, 3] = [batch_size, 1, 3]
        context_attn = torch.bmm(expand_hidden_states, context.permute(0, 2, 1))
        context_attn = F.softmax(context_attn, dim=2)  # [batch_size, 1, 3]
        # [batch_size, 1, 3] * [batch size, 3, hidden_size] = [batch_size, 1, hidden_size]
