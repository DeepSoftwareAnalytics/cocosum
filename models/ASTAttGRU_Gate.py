...
class ASTAttnGRUDecoder(nn.Module):
    def __init__(self, vocab_size, emd_dim, hidden_size):
        ...
        self.predict = nn.Sequential(nn.Linear(hidden_size * 5, hidden_size),
                                     nn.ReLU(inplace=True),
                                     nn.Linear(hidden_size, vocab_size))
        
        self.gate_uml = nn.Sequential(nn.Linear(hidden_size * 2, hidden_size), nn.Sigmoid())
        self.gate_name = nn.Sequential(nn.Linear(hidden_size * 2, hidden_size), nn.Sigmoid())
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        # [batch_size, 1, hidden_size]
        class_name_gate = self.gate_name(torch.cat((class_feature, expand_hidden_states), dim = 2))
        # [batch_size, 1, hidden_size]
        uml_gate = self.gate_name(torch.cat((class_encoder_output, expand_hidden_states), dim = 2))
        
        class_name = torch.mul(class_name_gate, class_feature)
        uml = torch.mul(uml_gate, class_encoder_output)
        
       
        final_context = torch.cat((class_name, uml, ast_context, txt_context, expand_hidden_states), dim=2)
        # [batch_size, hidden_size * 5]
