...
class HDeepComModelUML(nn.Module):
    ...
    def forward(self, method_code, method_sbt, beam_width, is_test, uml_data = None, class_index = None, uml_index = None):
        """
        ...
        if beam_width > 0 and is_test and cf.beam_search_method != "none":
            ...
            for token_index in range(1, self.summary_len):
                ...
                class_context_attn = torch.bmm(pre_summary_hidden_state.unsqueeze(1), class_query.permute(0, 2, 1))
                ...
                class_vector = torch.bmm(class_context_attn, class_value).view(-1, self.hidden_size)

                context = code_context_vector + ast_context_vector + class_vector
