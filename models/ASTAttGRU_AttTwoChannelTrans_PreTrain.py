...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        class_query = torch.cat((self.query_trans_uml(class_encoder_output), self.query_trans_name(class_feature)),
                                dim=1)
        class_value = torch.cat((self.value_trans_uml(class_encoder_output), self.value_trans_name(class_feature)),
                                dim=1)
        # pdb.set_trace()
...
class AstAttGRUModelUML(nn.Module):
    ...
    def forward(self, method_code, source_mask, method_summary, use_teacher, method_sbt=None, uml_data=None, class_index=None,
        ...
        token_encoder_output = output[0].contiguous()
        ...
        decoder_outputs = torch.zeros(int(cf.batch_size/2), summary_length - 1, self.decoder.output_size)
