...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        debug_logger("ASTAttGRUDecoder (1): summary_embedding.shape %s, hidden_states.shape %s" % (
        ...
        class_context = torch.cat((class_encoder_output, class_feature), dim=1)
        # pdb.set_trace()
        # [batch_size, 1, hidden_size] * [batch size, hidden_size, 2] = [batch_size, 1, 2]
        class_context_attn = torch.bmm(expand_hidden_states, class_context.permute(0, 2, 1))
        ...
        class_context = torch.bmm(class_context_attn, class_context)
...
class AstAttGRUModelUML(nn.Module):
    def __init__(self, code_vocab_size, sbt_vocab_size, summary_vocab_size, encoder, enable_sbt=True, enable_uml=True):
        ...
        # self.fc = nn.Linear(cf.code_len*768, cf.code_len*cf.rnn_hidden_size)
        self.pool = nn.MaxPool1d(kernel_size=3, stride=3, padding=0)

    def forward(self, method_code, source_mask, method_summary, use_teacher, method_sbt=None, uml_data=None, class_index=None,
                uml_index=None):
        ...
        '''用pooling层降维'''
        output = self.code_encoder(method_code, attention_mask=source_mask)[0].contiguous()  # batch_size, 100, 768
        token_encoder_output = self.pool(output)
