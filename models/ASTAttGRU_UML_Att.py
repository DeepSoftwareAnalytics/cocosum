...
class ASTAttnGRUDecoder(nn.Module):
    ...
    def forward(self, method_summary_token, sbt_encoder_hidden, code_encoder_hidden, sbt_encoder_output,
        ...
        context = torch.cat((txt_context, ast_context, class_encoder_output, class_feature), dim=1)
        #pdb.set_trace()
        # [batch_size, 1, hidden_size] * [batch size, hidden_size, 4] = [batch_size, 1, 4]
        context_attn = torch.bmm(expand_hidden_states, context.permute(0, 2, 1))
        context_attn = F.softmax(context_attn, dim=2)  # [batch_size, 1, 4]
        # [batch_size, 1, 4] * [batch size, 4, hidden_size] = [batch_size, 1, hidden_size]
        fusion_context = torch.bmm(context_attn, context)
        
       
        final_context = torch.cat((fusion_context, expand_hidden_states), dim=2)
        # [batch_size, hidden_size * 2]
        final_context = final_context.view(context.shape[0], -1)
