        ...
        if enable_uml:
            if cf.class_only:
                self.decoder = ASTAttnGRUDecoder(vocab_size=summary_vocab_size, emd_dim=cf.summary_dim,
                                                 ...
                                                 class_output_dim = cf.gnn_num_hidden[-1])
            elif cf.gnn_concat and cf.gnn_concat_uml:
                self.decoder = ASTAttnGRUDecoder(vocab_size=summary_vocab_size, emd_dim=cf.summary_dim,
                                                 hidden_size=cf.rnn_hidden_size, enable_sbt = enable_sbt,
                                                 enable_uml = enable_uml, 
                                                 class_output_dim = cf.gnn_num_hidden[-1] * 2 + cf.gnn_num_feature)
                
            elif cf.gnn_concat:
                self.decoder = ASTAttnGRUDecoder(vocab_size=summary_vocab_size, emd_dim=cf.summary_dim,
                                                 hidden_size=cf.rnn_hidden_size, enable_sbt = enable_sbt,
                                                 enable_uml = enable_uml, 
                                                 class_output_dim = cf.gnn_num_hidden[-1] + cf.gnn_num_feature)
            elif cf.gnn_concat_uml:
                self.decoder = ASTAttnGRUDecoder(vocab_size=summary_vocab_size, emd_dim=cf.summary_dim,
                                                 hidden_size=cf.rnn_hidden_size, enable_sbt = enable_sbt,
                                                 enable_uml = enable_uml, 
                                                 class_output_dim = cf.gnn_num_hidden[-1] * 2)
            else:
                self.decoder = ASTAttnGRUDecoder(vocab_size=summary_vocab_size, emd_dim=cf.summary_dim,
                                                 hidden_size=cf.rnn_hidden_size, enable_sbt = enable_sbt,
                                                 enable_uml = enable_uml, 
